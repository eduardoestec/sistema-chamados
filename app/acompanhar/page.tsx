        ...
        <div className='bg-white rounded-xl shadow-sm p-6 mb-6'>
          <div className='flex gap-3'>
            ...
            <button
              onClick={buscar}
              disabled={buscando}
              className='bg-[#604404] hover:bg-[#4a3203] text-white font-medium px-6 py-3 rounded-lg transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed'
            >
              {buscando ? '...' : 'Buscar'}
            </button>
          </div>
        ...
        {chamado && (
          <div className='bg-white rounded-xl shadow-sm p-6'>
              ...
              {statusOrdem.map((s, index) => {
                ...
                return (
                  <div key={s} className='flex items-start gap-4 mb-6 relative'>
                    {/* Linha vertical */}
                    {!isLast && (
                      <div className={`absolute left-2 top-6 w-0.5 h-12 ${atual || ativo ? 'bg-[#604404]' : 'bg-[#e5e3e3]'}`} />
                    )}

                    {/* Círculo */}
                    <div className={`w-4 h-4 rounded-full mt-0.5 flex-shrink-0 ${atual ? 'bg-[#604404]' : ativo ? 'bg-[#16a34a]' : 'bg-[#e5e3e3]'}`} />

                    {/* Conteúdo */}
                    <div className='flex-1'>
                      <p className={`text-sm font-medium ${atual ? 'text-[#604404]' : ativo ? 'text-[#1a1a1a]' : 'text-[#6b7280]'}`}>
                        {statusLabel[s]}
                      </p>
                      {item && (
                      ...
                      {item && item.observacao && (
                        <p className='text-xs text-[#6b7280] mt-2 bg-[#fdf8f0] rounded-lg p-3'>{item.observacao}</p>
                      )}
                    </div>
                  </div>
                )
            ...
            {chamado.previsao_resolucao && (
              <div className='mt-6 bg-[#fdf8f0] rounded-xl p-4'>
