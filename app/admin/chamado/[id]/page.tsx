...
export default function DetalheChamado({ params }: { params: { id: string } }) {
  ...
  async function carregarChamado(id: string) {
    ...
    setChamado(c as Chamado)
    setHistorico(h || [])
    setNovoStatus((c as Chamado).status)
  }
  ...
  async function salvar() {
    ...
    setObservacao('')
    await carregarChamado(params.id)
    setSalvando(false)
        ...
        <div className='grid grid-cols-2 gap-2 mb-4'>
          {statusOrdem.map(s => (
            <button
              ...
              className={
                novoStatus === s
                  ? 'py-2 px-3 rounded-lg border text-sm font-medium bg-yellow-400 border-yellow-400 text-gray-900'
                  : 'py-2 px-3 rounded-lg border text-sm font-medium bg-white border-gray-200 text-gray-600'
              }
            >
              {statusLabel[s]}
            </button>
          ...
          {historico.map(h => (
            <div key={h.id} className='flex items-start gap-3'>
              ...
              <div>
                ...
                {h.observacao && (
                  <p className='text-xs text-gray-500 mt-1'>{h.observacao}</p>
                )}
              </div>
            </div>
          ))}
